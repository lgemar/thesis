...
% margin adjustment
\usepackage[a4paper,right=35mm,left=35mm,top=26mm,bottom=26mm]{geometry} % https://www.sharelatex.com/blog/2013/08/06/thesis-series-pt2.html
...
Not all of these three tracking targets are measured directly. The tracking targets are inferred from the measurements of the position of the rear of the interaction tool relative to the camera, the attitude of the camera, and the attitude of the tool. The position of the tool relative to the camera is denoted by $\prpr$, the attitude of the camera relative to the inertial coordinate system by $\Ac$ and the attitude of the tool relative to the inertial coordinate system by $\At$. The vector $\prpr$ is measured by finding the position of the tracking target in the camera field of view and measuring its size to determine its depth. Using its depth and its position in the field of view, its three-dimensional position relative to the camera can be estimated. This model will be presented in more detail in a later section of the report.
...
The Kalman filter assumes process noise and measurement noise in the dynamics. The process noise for the dynamic model of rotations assumes that the body is fixed at each time step, so the noise in the dynamics must come from noise on the gyroscope input. The root-mean squared noise density of gyroscope on the Adafruit BNO055 given in the data sheet as $0.014 \frac{\frac{\degtwo}{s}}{\sqrt{\Hz}}$ \cite{bnO055datasheet}. To find the mean squared noise, this value is squared and multiplied by bandwidth of the signal, taken here to be the sample rate, $47$ Hz. This yields a value of approximately $(0.1)^2 \frac{\degtwo}{s}$. Converting to radians, the mean squared noise of the gyroscope is $2.8\mathrm{e}{-6} \frac{rad}{s}^2$. Thus, the process noise $Q$ is initialized as 

$$ Q = \mat{2.8\mathrm{e}{-6} & 0 & 0 \\ 0 & 2.8\mathrm{e}{-6} & 0 \\ 0 & 0 & 2.8\mathrm{e}{-6}} $$

\medskip

Similarly, the measurement noise is initialized with noise values for the accelerometer and magnetometer listed in the datasheet \cite{bnO055datasheet}. Again, squaring the noise densities and multiplying by the bandwidths yields mean squared errors of $1.1\mathrm{e}{-6} g^2$ and $17 \mu T^2 $ for the accelerometer and the magnetometer, respectively. The six dimensional measurement noise matrix, $R$, is initialized as 

$$ R = \mat{1.1\mathrm{e}{-6} & 0 & 0 & 0 & 0 & 0 \\
            0 & 1.1\mathrm{e}{-6} & 0 & 0 & 0 & 0 \\
            0 & 0 & 1.1\mathrm{e}{-6} & 0 & 0 & 0 \\
            0 & 0 & 0 & 17 & 0 & 0 \\
            0 & 0 & 0 & 0 & 17 & 0 \\
            0 & 0 & 0 & 0 & 0 & 17} $$
...
Intuitively, a larger value for the measurement noise means that the variability of the measurements is high, so the filter is less responsive to measurements and more responsive to the model. Conversely, if the measurement noise parameter decreases, the Kalman gain increases. The graph in figure \ref{fig:rtuning} shows how the Kalman filter responds to a step input in the roll for different values of $R$, the measurement gain. 
...
\begin{figure}[H]
    \begin{center}$
        \begin{array}{cc}
            \includegraphics[width=0.8\textwidth]{KalmanBias} \\
            \includegraphics[width=0.8\textwidth]{KalmanVariance}
        \end{array}$
    \end{center}
...
\begin{figure}[H]
    \begin{center}$
        \begin{array}{cc}
            \includegraphics[width=0.8\textwidth]{FilterComparison}
        \end{array}$
    \end{center}
...
Note that the angular error target of $5\degtwo$ rmse is achieved in each maneuver direction except for yaw. As the chart shows, the Kalman Filter reduces the mean-squared-error of the orientation estimates by a factor of more than four versus the naive TRIAD estimation method.
...
The output of the position tracking filter is the position of the rear of the interaction tool $\pr$. Tracking this position with low variance is important to produce an accurate estimation for the position of the front of the interaction tool.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Naive measurement model
\subsection{Measurement Model}

The following is a model of a single frame from the camera. In this frame, shown in image Figure \ref{fig:computervis} the red circular object represents the rear of the interaction tool. The spherical object of the rear of the interaction tool is called the tracking target. The width of the target along the $x$ dimension of the image is $s_{u}$. 
...
These relationships are derived by using a pin-hole camera model as shown in \cite{Tsai1987}. 

\subsubsection{Camera Calibration}

The measurement model requires knowledge of the intrinsic camera parameters, $f_x, f_y, c_x, c_y$, and the radial and tangential distortion factors. There are two steps to translating a point from camera coordinates to image coordinates \cite{Tsai1987}. First, the pin-hole camera model shows how to find the ideal image coordinates of an object, $(x_u, y_u)$ based on the position of the object in camera coordinates, $\prpr = (x, y, z)$: 
...
$$ x_u = x_d + (2 p_1 x y + p_2 (r^2 + 2x^2))$$
$$ y_u = y_d + (p_1 (r^2 + 2y^2) + 2p_2 x y)$$
...
The natural conclusion from the traces in \ref{fig:depth} is that the depth tracking variance depend on how far away the interaction tool is from the tracking camera.

\subsection{Error Analysis}

\subsubsection{Motion in the Image Plane}

The precision of the tracking system depends on the direction of the tool maneuvers. This is evident from analyzing the covariance of the error in the $y$ and $z$ directions for vertical and horizontal maneuvers. Figure \ref{fig:yzerror} shows the error ellipse associated with motion in the image plane. On the left is the error ellipse associated with vertical motion and on the right is the error associated with horizontal motion. 
...
\subsubsection{Summary of measurement errors}

The naive tracking model above has errors for the three maneuvers as summarized by the tables below in Figure \ref{fig:naiveposition}. 

\begin{figure}[H]
    \begin{center}$
        \begin{array}{cc}
            \includegraphics[width=0.7\textwidth]{NaivePositionBias} \\
            \includegraphics[width=0.7\textwidth]{NaivePositionError}
        \end{array}$
    \end{center}
    \caption{Bias and variance for position measurements}
    \label{fig:naiveposition}
\end{figure}
...
Since the user's movement of the interaction tool is voluntary, no assumptions can be made about its overall trajectory. However, human-motion obeys minimum-jerk constraints \cite{flash1985}. Since the jerk of the tool's motion is small, it can be modeled as a white-noise process. The dynamical model for the tool's motion along the $x$ axis is estimated by the discrete time state-space model

$$ \vec{x}_{k+1} = \mat{1 & \Delta t \\ 0 & 1} \mat{x \\ \xd} + \mat{(\Delta t)^2/2 \\ \Delta t} a_x  + \mat{0 \\ 1} w_k $$
...
Using this dynamic model for integrating the position estimates from the camera and the acceleration measurements from the accelerometer, slight improvements in the position estimation are obtained. These are summarized in the tables of Figure \ref{fig:kalmanposition}: 

\begin{figure}[H]
    \begin{center}$
        \begin{array}{cc}
            \includegraphics[width=0.7\textwidth]{KalmanPositionError} \\
            \includegraphics[width=0.7\textwidth]{KalmanPositionVariance}
        \end{array}$
    \end{center}
    \caption{Bias and variance for filtered position estimates}
    \label{fig:kalmanposition}
\end{figure}

The comparison of these estimation errors to those of the simpler measurement model is shown in Figure \ref{fig:positionerrorcomp}.

\begin{figure}[H]
    \begin{center}$
        \begin{array}{c}
            \includegraphics[width=0.8\textwidth]{PositionFilterComparison}
        \end{array}$
    \end{center}
    \caption{Comparison of position estimators}
    \label{fig:positionerrorcomp}
\end{figure}

Notice that the filtered estimates are not any better (and in some cases worse) that the naive estimates. This could be for a couple reasons. The first is that the accelerometer measurements are too noisy to provide data that is more reliable than the position measurements from the image frames. It turns out that the sample rate of the accelerometer is 47Hz while the sample rate of the images is 60Hz. This means that the measurements from the images are potentially more responses to changes in the tool's acceleration than the accelerometer itself. Second, the accelerometer is subject to errors in the orientation when its measurements are converted from body coordinates to world coordinates. It could be that too much error is occurred in this rotation to make them useful in improving the state prediction. 
...
Plugging in this expression for $\A(\dq)$: 
...
Revisiting the design objectives in \ref{fig:performance}, the table in Figure \ref{fig:summary1} captures the progress made on the performance of the tracking system. In green are those specifications that were achieved, and in orange are the specifications towards which progress was made. 
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Usability design goals, revisited}
    \label{fig:summary2}
\end{figure}
...
There are two angles to approaching the future of The 3D Interaction Tool: one approach is technical and the other approach is focused on the end-user of the tool.

\medskip

As shown in Figure \ref{fig:overallerror}, the biggest area for technical improvement -- that is improvement of the tracking apparatus -- is decreasing the position estimation error. The first technical proposal is to make the estimation more robust to tool movements by improving the image segmentation process. Modifying the image segmentation algorithm itself could accomplish this. One way to improve the image segmentation would be to map the accelerometer measurements into predicted movements in the image plane to reduce the uncertainty of the tool's position when it moves from side-to-side or up and down. The second way to improve the image segmentation process is to improve the geometry of the problem. Using a camera with a higher resolution or faster frame-rate could reduce the blurring that leads to uncertainty of position with motion. 

\medskip

The second proposal to improving the position estimates is to improve the depth tracking by adding a more recognizable feature to the back of the interaction tool. While color was an acceptable target for the purposes of this report, something like a QR code could be tracked more robustly. This approach may also have the advantage of providing another state measurement to also decrease the uncertainty of the orientation estimation algorithm. 

\medskip

Finally, future work on the 3D Interaction Tool would also include a usability study to assess how the performance parameters affect the end-user. A study that assesses how well a user is able to perform simple object manipulation tasks would go a long way to improving the design of the device.

\section{Acknowledgements}

I would like to thank my advisers Avi and Professor Gajos for their support and advice throughout the completion of this report. I would also like to thank Professor Rob Wood for allowing me to use the VICON motion tracking system in his lab to collect ground-truth measurements about the position and orientation of the interaction tool.
