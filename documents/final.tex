...
\subsubsection{State and Covariance}

The initial orientation is estimated using the TRIAD approach. The rotation matrix output of the TRIAD approach is converted to a quaternion to initialize the dynamic model. In order to inialize the covaraince, an initial uncertainty in the orientation of 10 degrees is assumed. 

\subsubsection{Process and Measurement Noise}
...
\subsubsection{First Order Error Analysis}

Given two random variables $X$ and $Y$, and the (potentially non-linear) relationship that $Y = f(X)$, how does the covariance of $Y$ change with the covariance of $X$? As a first-order approximation, the randomness of $Y$ can be approximated as a linear function of $X$: $Y - \mu_Y = T (X - \mu_X) $ \cite{haralick1996}. In the linearization of $f(X)$ about the mean of $X$, $\mu_X$, $T$ is the Jacobian matrix, $T = \frac{\partial Y}{\partial X}$. Now, given the linear relationship, $Y - \mu_Y = T (X - \mu_X) $, the covariance of $Y$ is related to the covariance of $X$ by 

$$ \Sigma_Y = T \Sigma_X T^{T} $$

This relationship is a general result about how covariances transform under linear operators and will be used extensively below.
